...
# Major cities by state for realistic locations
state_cities = {
...
for state, store_count in state_distribution.items():
    ...
    for i in range(int(store_count * SAMPLE_PCT)):
        ...
        # Vary store characteristics by trade area type
        trade_area = np.random.choice(['urban', 'suburban', 'university', 'mall'], 
        ...
        if trade_area == 'urban':
            ...
            parking = 0  # shared mall parking
            pop_3mi = random.randint(80000, 200000)
            income_3mi = random.randint(55000, 85000)
        
        store_format = np.random.choice(['standard', 'drive_thru', 'urban_compact'],
                                        p=[0.70, 0.20, 0.10])
